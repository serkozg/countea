...
<form method="GET" action="{{ route('laporan.pengeluaran-barang') }}">
...
<table class="table table-bordered">
    ...
    <tfoot>
        <tr class="total-row">
            <td colspan="3" class="text-bold text-center text-primary">Total Pengeluaran</td>
            <td class="text-bold text-danger">{{ number_format($totalPengeluaran, 0, ',', '.') }}</td>
        </tr>
    </tfoot>
</table>
...
<script>
    ...
    function openPrintWindow() {
        ...
        printWindow.document.write('<html><head><title>Laporan Barang Keluar</title>');
        ...
        printWindow.document.write('<h1>Laporan Barang Keluar</h1>');
        printWindow.document.write('<table class="table" border="1" cellpadding="8" cellspacing="0">');
        printWindow.document.write(
            '<thead><tr><th>ID</th><th>Tanggal</th><th>Nama Barang</th><th>Jumlah Pengeluaran</th></tr></thead><tbody>');

        @foreach ($pengeluaran as $item)
            printWindow.document.write(
                '<tr><td>{{ $loop->iteration }}</td><td>{{ $item->tanggal }}</td><td>{{ $item->barang->nama_barang }}</td><td>{{ number_format($item->jumlah, 0, ',', '.') }}</td></tr>'
                );
        @endforeach

        printWindow.document.write('</tbody><tfoot>');
        printWindow.document.write(
            '<tr class="total-row"><td colspan="3">Total Pengeluaran</td><td>{{ number_format($totalPengeluaran, 0, ',', '.') }}</td></tr>'
            );
